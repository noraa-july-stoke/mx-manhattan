use crate::db_types::{AboutContent, Blog, Fan, PressBlurb, Release, Show, Video};
use axum::{
    extract::Extension,
    http::StatusCode,
...
async fn get_press(
    Extension(db_pool): Extension<PgPool>,
) -> Result<Json<Vec<PressBlurb>>, StatusCode> {
...
async fn get_about(Extension(db_pool): Extension<PgPool>) -> Result<impl IntoResponse, StatusCode> {
    let result = sqlx::query_as!(
        AboutContent,
        "SELECT id, bio, contact_email, designer, portfolio_link, created_at, updated_at FROM about LIMIT 1"
    )
    .fetch_one(&db_pool)
    .await;
    ...
    match result {
        ...
        Err(e) => {
            println!("Failed to retrieve the about data: {}", e);
